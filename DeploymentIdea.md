...
| Auth database | Supabase (managed PostgreSQL) | Stores users, roles, orgs, and memberships. Supabase is only the database here - authentication and JWT issuance are handled by Clerk, and the auth logic runs in the Go service |
...
4. ECS Fargate picks up the new image and redeploys that service independently. Each service gets its own environment variables (e.g., `CLERK_SECRET_KEY`, `MONGO_URI`, `ZOOM_API_KEY`).
...
6. Connect the repo to Vercel for the frontend. It reads environment variables for the API gateway URL and Clerk publishable keys, and rebuilds automatically on push.

**3. Security**

- On login, Clerk issues a signed JWT. Every backend service verifies it independently before doing anything - no service trusts a request without a valid token.
- The Go auth service enforces organization-scoped access control, ensuring users can only touch data from their own organization (multi-tenant isolation).
